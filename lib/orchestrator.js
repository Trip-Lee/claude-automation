...
export class Orchestrator {
  ...
  async runClaudeCodeAgentSystem(container, description, costMonitor, projectPath, options = {}) {
    ...
    // Generate orchestrator summary of the task
    await this.generateTaskSummary(conversation, executor.getSummary(), description, costMonitor);
...
Analyze this task and create an execution plan. Output your plan in the JSON format specified.`;
    ...
    try {
      ...
      if (plan) {
        ...
        const isDirectCompletion = plan.taskType && (
          ...
          plan.taskType === 'completed' ||
          plan.taskType === 'none' ||
          plan.taskType === 'simple'
        );
   ...
   * Uses the orchestrator to review what agents did and provide a wrap-up
   * @param {ConversationThread} conversation - The conversation history
   * @param {Object} executionSummary - Summary from the executor
   * @param {string} taskDescription - Original task description
   * @param {CostMonitor} costMonitor - Cost tracking
   */
  async generateTaskSummary(conversation, executionSummary, taskDescription, costMonitor) {
    const { ClaudeCodeAgent } = await import('./claude-code-agent.js');

    // Get conversation history for the summary
    const history = conversation.getCondensedHistory();

    // Skip summary if only one agent ran briefly
    if (executionSummary.totalAgents <= 1 && executionSummary.totalDuration < 10000) {
      return; // Simple task, no need for elaborate summary
    }
    ...
    try {
      // Create a fast summarizer agent
      const summarizerAgent = new ClaudeCodeAgent({
        role: 'orchestrator',
        model: 'haiku', // Fast summarization
        allowedTools: [], // No tools needed
        systemPrompt: `You are the orchestrator summarizing a completed task. Be concise and informative.

Your summary should be 2-4 sentences covering:
1. What was accomplished
2. Key actions taken by agents
3. Final outcome

Do NOT use markdown headers or bullet points. Just write a brief paragraph.`,
        sessionId: uuidv4()
      });

      const summaryPrompt = `Task: ${taskDescription}

Agents that worked: ${executionSummary.agentSequence.join(' → ')}
Duration: ${(executionSummary.totalDuration / 1000).toFixed(1)}s

Conversation:
${history}

Write a brief 2-4 sentence summary of what was accomplished.`;

      // The streaming output already displays the summary
      await summarizerAgent.executeWithTools({
      ...
      console.log(chalk.white(`  Completed via ${executionSummary.agentSequence.join(' → ')}`));
      console.log(chalk.white(`  ${executionSummary.totalAgents} agent(s) executed in ${(executionSummary.totalDuration / 1000).toFixed(1)}s`));
    }
