...
export class Orchestrator {
  ...
  async executeTask(projectName, description) {
    ...
    try {
      ...
      if (this.githubClient && config.repo && config.repo !== 'local') {
        ...
        if (!config.repo.includes('github.com/')) {
          throw new Error(
            ...
            `Update config at: ~/.claude-projects/${projectName}.yaml`
          );
        }
        ...
        if (!repoExists) {
          throw new Error(
            ...
            `  3. Or update config: ~/.claude-projects/${projectName}.yaml`
          );
        }
      ...
      // Remove from active tracking now that PR creation is complete
