 ...
 *
 * Handles loading YAML configuration files from configured directory
...
export class ConfigManager {
  constructor() {
    ...
    this.configDir = globalConfig.get('configDir');
  }
  ...
  validate(config) {
    ...
    if (config.repo !== 'local' && !this.isValidRepoUrl(config.repo)) {
      throw new Error(
        ...
        `Update config at: ${path.join(this.configDir, `${config.name}.yaml`)}`
      );
    }
  ...
  applyDefaults(config) {
    ...
    return {
      ...
      docker: {
        ...
        memory: config.docker.memory || process.env.DEFAULT_DOCKER_MEMORY || globalConfig.get('docker.defaultMemory') || '4g',
        cpus: config.docker.cpus || parseInt(process.env.DEFAULT_DOCKER_CPUS || globalConfig.get('docker.defaultCpus') || '2'),
      ...
      safety: {
        max_cost_per_task: config.safety?.max_cost_per_task || parseFloat(process.env.DEFAULT_MAX_COST || globalConfig.get('safety.maxCostPerTask') || '5.00'),
