...
export class CostMonitor {
  ...
  addUsage({ inputTokens, outputTokens }) {
    ...
    // Skip cost checks if maxCost is 0 (unlimited)
    if (this.maxCost > 0) {
      // Check if approaching limit (75% threshold)
      if (this.totalCost >= this.maxCost * 0.75 && this.totalCost < this.maxCost) {
        console.log(chalk.yellow(
          `[WARN]  Approaching cost limit: $${this.totalCost.toFixed(2)} / $${this.maxCost.toFixed(2)}`
        ));
      }
  ...
  async save(taskId) {
    ...
    const costDir = path.join(globalConfig.get('logsDir'), 'costs');
