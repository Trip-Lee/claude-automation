...
Perform your work and make your decision.`;
  ...
  getAvailableExecutionContext() {
    try {
      ...
      context += `  MCP Tool: query_execution_context (includes impact_chain)\n`;
      context += `  Returns: Business Rules, fields auto-set, cascading records, risk level\n`;
      context += `  Impact Chain: Triggering flows, referencing flows, cascading tables\n`;
