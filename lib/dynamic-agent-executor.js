...
${fileCacheSummary !== 'No files cached yet.' ? `\n**Cached Files:**\n${fileCacheSummary}\n` : ''}

**IMPORTANT - Next Step Decision:**

After completing your work, you MUST decide the next step by including this at the END of your response:
...
**Decision Guidelines:**
1. **NEXT: COMPLETE** - Use if task is fully finished and no more work needed
2. **NEXT: [agent]** - Hand off to another agent if more work required
3. Consider what makes sense:
   - After analysis (architect) → usually coder or reviewer
   - After coding (coder) → usually reviewer or security
   - After review (reviewer) → usually COMPLETE or back to coder if issues
4. Don't loop unnecessarily - if work is done, mark COMPLETE
5. Be specific in your reasoning

**Examples:**
- \`NEXT: coder | REASON: Implementation plan ready, code needs to be written\`
- \`NEXT: reviewer | REASON: Code changes complete, needs quality review\`
- \`NEXT: security | REASON: Auth code complete, needs security validation\`
- \`NEXT: COMPLETE | REASON: Analysis finished, no code changes needed\`
- \`NEXT: COMPLETE | REASON: All requirements met and approved\`

Now perform your work and make your decision.`;
  }
