...
export class ConversationThread {
  ...
  /**
   * Get condensed conversation history (optimized for brevity)
   * @returns {string} - Condensed conversation summary
   */
  getCondensedHistory() {
    ...
    // Condense each message to key points only
    const condensed = this.messages
      .map(msg => {
        // Extract key information from message
        const content = msg.content;

        // For long messages, extract just the decision/summary
        let summary;
        if (content.length > 300) {
          // Try to extract NEXT/REASON or last paragraph
          const nextMatch = content.match(/NEXT:\s*([^\n]+)/i);
          const reasonMatch = content.match(/REASON:\s*([^\n]+)/i);

          if (nextMatch && reasonMatch) {
            summary = `${nextMatch[1].trim()} - ${reasonMatch[1].trim()}`;
          } else {
            // Get last meaningful paragraph (likely the summary)
            const paragraphs = content.split('\n\n').filter(p => p.trim().length > 0);
            summary = paragraphs[paragraphs.length - 1].substring(0, 200);
          }
        } else {
          summary = content;
        }
