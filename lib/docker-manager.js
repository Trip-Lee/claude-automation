...
export class DockerManager {
   ...
   * @param {Object} volumes - Volume mappings
   * @returns {Array<string>} - Bind mount strings
   */
  createBinds(volumes) {
    // SECURITY: Create bind mounts
    // CRITICAL: Only mount the specific project directory
    // NEVER mount:
    // - ~/claude-automation (orchestrator code)
    // - ~/.env (credentials)
    ...
    return Object.entries(volumes).map(
      ([hostPath, containerPath]) => {
        ...
        const forbidden = [
          ...
          '/home/coltrip/.env',
          '/home/coltrip/.claude-projects',
          '/home/coltrip/.claude-tasks',
          '/root'
        ];
