...
export class DockerManager {
    ...
    return new Promise((resolve, reject) => {
      exec.start({ hijack: true, stdin: false }, (err, stream) => {
        ...
        stream.on('end', async () => {
          ...
          // Return structured result instead of just string
          resolve({
            exitCode: inspectData.ExitCode,
            stdout: output,
            stderr: '' // Docker multiplexes streams, stderr is in output
          });
        });
   ...
   * @param {Container} container - Docker container
   * @returns {Promise<string>} Status ('running', 'exited', 'paused', etc.)
   */
