...
export class GitHubClient {
  ...
  async updatePullRequest({ repo, prNumber, title, body }) {
    ...
    try {
      ...
      throw this._enhanceGitHubError(error, 'update pull request');
    }
  }
  ...
  async closePullRequest({ repo, prNumber }) {
    ...
    try {
      ...
      throw this._enhanceGitHubError(error, 'close pull request');
    }
  }
  ...
  parseRepo(repoUrl) {
    ...
    if (parts.length < 2) {
      throw new Error(
        ...
        `Expected format: github.com/owner/repo, https://github.com/owner/repo, or git@github.com:owner/repo`
      );
    }
    ...
    try {
      ...
      if (error.status === 403) {
        throw new Error(
          ...
          `Check your token permissions at https://github.com/settings/tokens`
        );
      }
      throw this._enhanceGitHubError(error, 'check repository access');
    }
  }

  /**
   * Enhance GitHub API errors with helpful messages
   * @param {Error} error - Original error
   * @param {string} operation - Operation that failed
   * @returns {Error} - Enhanced error
   * @private
   */
  _enhanceGitHubError(error, operation) {
    const status = error.status || error.response?.status;

    switch (status) {
      case 401:
        return new Error(
          `GitHub authentication failed while trying to ${operation}.\n\n` +
          `Your GITHUB_TOKEN is invalid or expired.\n\n` +
          `To fix this:\n` +
          `  1. Go to https://github.com/settings/tokens\n` +
          `  2. Generate a new token with 'repo' scope\n` +
      ...
      case 403:
        return new Error(
          `GitHub access forbidden while trying to ${operation}.\n\n` +
          `This could be due to:\n` +
          `  - Rate limit exceeded (wait 1 hour)\n` +
      ...
      case 422:
        return new Error(
          `Validation failed while trying to ${operation}.\n\n` +
      ...
      case 503:
        return new Error(
          `GitHub server error while trying to ${operation}.\n\n` +
      ...
      default:
        return new Error(
          ...
          `If this persists, check https://www.githubstatus.com/`
        );
    }
  }
  ...
  async getPRStatus({ repo, prNumber }) {
    ...
    try {
      ...
      throw this._enhanceGitHubError(error, 'get PR status');
    }
  }
