...
export class GitManager {
  ...
  async pull(projectPath, branch) {
    ...
    try {
      ...
      try {
        ...
        // No remote configured - this is a local-only repository
        console.log(chalk.yellow(`  ⚠️  No remote 'origin' configured (local-only repository)`));
        console.log(chalk.green(`  ✅ On branch ${branch}`));
        return 'Local repository - no remote';
      }
   ...
   * Get git diff
   * @param {string} projectPath - Path to git repository
   * @param {string|Object} baseBranchOrOptions - Base branch name or options object
   * @param {string} targetBranch - Target branch name (optional)
   * @param {boolean} statOnly - Show only diffstat (optional)
   * @returns {Promise<string>} - Git diff output
   */
  async getDiff(projectPath, baseBranchOrOptions = {}, targetBranch = null, statOnly = false) {
    try {
      let command = 'git diff';

      // Support both old and new signatures
      if (typeof baseBranchOrOptions === 'string') {
        // New signature: getDiff(projectPath, baseBranch, targetBranch, statOnly)
        if (statOnly) {
          command += ' --stat';
        }
        command += ` ${baseBranchOrOptions}`;
        if (targetBranch) {
          command += `..${targetBranch}`;
        }
      } else {
        // Old signature: getDiff(projectPath, options)
        const options = baseBranchOrOptions;
        if (options.staged) {
          command += ' --staged';
        }
        if (options.file) {
  ...
  async push(projectPath, branchName, setUpstream = true) {
    try {
      // Check if remote 'origin' exists
      const hasRemote = await this.hasRemote(projectPath, 'origin');

      if (!hasRemote) {
        throw new Error(
          `Git remote 'origin' not configured for ${projectPath}\n\n` +
          `To fix this:\n` +
      ...
      // Enhance error message for common push failures
      if (error.message.includes('remote origin already exists')) {
        throw new Error(`Remote 'origin' already exists. Cannot push.`);
      }
   ...
   */
  async hasRemote(projectPath, remoteName = 'origin') {
    try {
   ...
   */
  async getRemoteUrl(projectPath, remoteName = 'origin') {
    try {
      const { stdout } = await execAsync(`git remote get-url ${remoteName}`, { cwd: projectPath });
      return stdout.trim();
    } catch (error) {
      return null;
    }
