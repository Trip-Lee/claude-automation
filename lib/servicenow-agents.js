...
⛓️ IMPACT CHAIN ANALYSIS (REQUIRED):

When working with tables that flows affect, ALWAYS check the full impact chain:

[ ] Use \`query_execution_context\` MCP tool - returns BRs, flows, cascading effects
    Example: mcp__sn-tools__query_execution_context({ table_name: "incident", operation: "insert" })

The Impact Chain shows:
- **Triggering Flows**: Flows that auto-trigger on this table's operations
- **Referencing Flows**: Flows that read/write this table (called from elsewhere)
- **Business Rules**: Before/after/async phases that fire
- **Cascading Tables**: Child records, references that may be affected
- **Risk Level**: LOW/MEDIUM/HIGH based on automation complexity

This helps you understand the FULL automation sequence when records change.`,
        standards: {
          includeEffort: true,
          includeSecurity: true,
          includePerformance: true,
...
When your scripts CREATE, UPDATE, or DELETE records, ALWAYS check the full impact chain:

[ ] Use \`query_execution_context\` MCP tool - returns BRs, flows, cascading effects
    Example: mcp__sn-tools__query_execution_context({ table_name: "incident", operation: "update" })

The Impact Chain shows:
- **Triggering Flows**: Flows that auto-trigger when your script modifies records
- **Referencing Flows**: Flows that also access this table (potential conflicts)
- **Business Rules**: Before/after/async phases that fire after your script's operations
- **Cascading Tables**: Child records, references that cascade from your changes
- **Risk Level**: LOW/MEDIUM/HIGH based on automation complexity

CRITICAL: Your script's GlideRecord operations may trigger ADDITIONAL automation!
Always understand the full cascade before committing changes.`,
        standards: {
          includeEffort: true,
          includeSecurity: true,
          includePerformance: true,
