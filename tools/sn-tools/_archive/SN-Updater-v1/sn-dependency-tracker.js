...
class ServiceNowDependencyTracker {
    constructor() {
        ...
        this.tenonRepoPath = path.join(__dirname, '../Tenon Repo');
        this.serviceNowDataPath = path.join(__dirname, '../ServiceNow Data/Tenon');
    ...
    async scanComponents() {
        ...
        const componentDirs = [
            path.join(this.tenonRepoPath, 'Sashimono'),
            path.join(this.tenonRepoPath, 'component-library')
        ];
    ...
    findApiCalls(content, componentDeps) {
        ...
        while ((match = apiPathRegex.exec(content)) !== null) {
            ...
            if (apiPath.includes('/api/now/') || this.isTenonApiPath(apiPath)) {
                componentDeps.apis.add(apiPath);
            }
        }
    }
    ...
    getComponentType(componentPath) {
        if (componentPath.includes('Sashimono')) return 'Sashimono';
        if (componentPath.includes('component-library')) return 'Component Library';
        return 'Unknown';
    }

    isTenonApiPath(path) {
        return path.includes('tenon') || path.includes('marketing') || path.includes('journey');
    }
